...
export function MarketingWorkspace({ workspace, canEdit }: Props) {
  ...
  async function handleLogo(file: File | undefined) {
    ...
    try {
      ...
      form.set("file", file);
      const result = await uploadBrandLogo(workspace.id, form);
      ...
      setError("Could not upload the logo file.");
    }
  }
  ...
  async function composeLogo(imageUrl: string) {
    ...
    try {
      const res = await fetch("/api/images/watermark", {
        ...
        body: JSON.stringify({ imageUrl, logoUrl, position: "top-left" }),
      });
      if (!res.ok) {
        console.error("Watermark request failed:", await res.text());
        return imageUrl;
      }
      const { watermarkedBase64 } = await res.json();
